  ...
  return instructions?.map((instruction) => (
    <div className="grid grid-cols-12 p-2 items-start" key={instruction?.index}>
      <div className="col-span-1">
        <IconsGuide type={instruction?.modifier} className="text-stone-800 dark:text-stone-200" />
      </div>
      <div className="col-span-11 ml-2 space-y-1">
        <p className="text-sm text-stone-800 dark:text-stone-200">{instruction?.text}</p>
        <div className="flex items-center gap-2">
          <span className="text-ss text-stone-700 dark:text-stone-300 grow whitespace-nowrap">
            {convertTime(instruction?.time)} ({convertDistance({ distance: instruction?.distance, type: 'm' })})
          </span>
          <div className="w-full h-[0.07rem] dark:bg-stone-500"></div>
        </div>
      </div>
    </div>
  ));
